...
bool move(int& i,int& j,int& di,int& dj, vector<string>& arr){
    int ni=i+di,nj=j+dj;
    int n=len(arr),m=len(arr[0]);
    if(ni<0 || ni>=n || nj<0 || nj>=m)return true;
    if(arr[ni][nj]=='#'){
        swap(di,dj);dj*=-1;
        return move(i,j,di,dj,arr);
    }i+=di;j+=dj;
    return false;
}

bool rec(ll& su, vll& arr, int i, ll cur){
    if(i==len(arr)){
        if(cur==su)return true;
        return false;
    }
    bool ans=rec(su,arr,i+1,cur+arr[i]) || rec(su,arr,i+1,cur*arr[i]);
    return ans;
}
...
bool rec2(ll& su, vll& arr, int i, ll cur, string curs){
    if(i==len(arr)){
        if(cur==su)return true;
        return false;
    }
    bool ans=rec2(su,arr,i+1,cur+arr[i], curs+'+') || rec2(su,arr,i+1,cur*arr[i],curs+'*') || rec2(su,arr,i+1,concat(cur, arr[i]),curs+'|');
    return ans;
}
...
void solve1(){
    ...
    while(getline(cin,line)){
        istringstream ss(line);
        string s;ss>>s;
        ll su=stoll(s.substr(0,len(s)-1));
        vll arr;int x;
        while(ss>>x){
            arr.pub(x);
        }
        int n=len(arr);
        if(rec(su,arr,1,arr[0]))ans+=su;
    }print(ans);
}
